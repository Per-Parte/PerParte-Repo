...
Configurador real em produção: [per-parte-web.vercel.app](https://per-parte-web.vercel.app) (`apps/web`), com modos Montar e Criar, geração de STL de produção, kit de calibração de encaixe (F5), links compartilháveis de criação, e interface em órbita ao redor da luminária. Motor de geometria (04/08/2026): revolução + pilha de estruturais + texturas + vazados (preview) + modulação por θ (facetas/squircle, encaixes sempre redondos) + corte de borda z(θ) (oblíquo/dentes) + espinha curva com contrapeso E3 + berço/gola no corpo + PLACA (refletor deitado, composição "luz + refletor") + cabeça inclinada (junta do gesto task light) — cobre 24 das 44 luminárias de mesa da Gantri (métrica de arquivo; ver nota "Cobertura" no cofre Obsidian). Próximas frentes: kit de validação impressa dos ⚑, contas e galeria de criações (marketplace), fronteiras caras de geometria (loft/casca/pivôs) quando o catálogo próprio pedir, decisões de negócio pendentes acima.
